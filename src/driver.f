      ...
      do nx1=nx0,nxN,nxD
         ...
         do nelt=iel0,ielN,ielD
           ...
           call h1mg_setup
              ...
              ! type castings of HSMG
              do ii=1,lxm*lxm
                  do jj=1,lmgn
                    mg_jh_f(ii,jj)=mg_jh(ii,jj)
                    mg_jht_f(ii,jj)=mg_jht(ii,jj)
                  enddo
              enddo
              do ii=0,lmgs*lmg_rwt*2*ldim*lelt-1
                  mg_rstr_wt_f(ii)=mg_rstr_wt(ii)
              enddo
              do ii=0,lmgs*lmg_fasts*2*ldim*lelt-1
                  mg_fast_s_f(ii)=mg_fast_s(ii)
              enddo
              do ii=0,lmgs*lmg_fastd*lelt-1
                  mg_fast_d_f(ii)=mg_fast_d(ii)
              enddo
              do ii=0,lmgs*lmg_swt*4*ldim*lelt-1
                  mg_schwarz_wt_f(ii)=mg_schwarz_wt(ii)
              enddo
              do ii=1,2*lxm*lym*lzm*lelt
                  mg_work_f(ii)=mg_work(ii)
              enddo
          ...
          ! call set_f(f,c,n)
          call set_f_ones(f,n)
!            open(unit=10, file='mat/f.csv', 
!      $          status='replace', action='write')
!            do count=1,N
!             write(10, '(f20.10)') f(count)
!            enddo
!            close(10)

          !  open(unit=10, file='w.csv', status='replace', action='write')
          !  do count=1,N
          !     write(10,*) f(count)
          !  enddo
          !  close(10)

          !  open(unit=10, file='z.csv', status='replace', action='write')
          !  do count=1,N
          !     write(10,*) f(count)
          !  enddo
          !  close(10)
           
      !      print *, nx1,ny1,nz1,nelt,n
...
c--------------------------------------------------------------
      ...
      do i=1,n
         ...
         f(i) = sin(arg)
      enddo
      ! call write2file(f,n,'f.dat')
      call dssum(f)
      ! call write2file(f,n,'f_dssum.dat')
      call col2 (f,c,n)
      ! call write2file(f,n,'f_setf.dat')
      return
      end
c-----------------------------------------------------------------------
      subroutine set_f_ones(f,n)
      real f(n)
