...
c-----------------------------------------------------------------------
      subroutine gop_f( x, w, op, n)
c
c     Global vector commutative operation in single precision
c
      include 'mpif.h'
      common /nekmpi/ nid,np,nekcomm,nekgroup
c
      real*4 x(n), w(n)
      character*3 op
c
      call adelay
      if (op.eq.'+  ') then
         call mpi_allreduce (x,w,n,mpi_real,mpi_sum ,nekcomm,ierr)
      elseif (op.EQ.'M  ') then
         call mpi_allreduce (x,w,n,mpi_real,mpi_max ,nekcomm,ierr)
      elseif (op.EQ.'m  ') then
         call mpi_allreduce (x,w,n,mpi_real,mpi_min ,nekcomm,ierr)
      elseif (op.EQ.'*  ') then
         call mpi_allreduce (x,w,n,mpi_real,mpi_prod,nekcomm,ierr)
      else
         write(6,*) nid,' OP ',op,' not supported.  ABORT in GOP.'
         call exitt
      endif
