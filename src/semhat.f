...
c-----------------------------------------------------------------------
      subroutine semhat_f(a,b,c,d,z,w,n)
c
c     Generate matrices for single element, 1D operators:
c
c        a    = Laplacian
...
c        z    = GLL points
      ...
      real*4 a(0:n,0:n),b(0:n),c(0:n,0:n),d(0:n,0:n),z(0:n)
      real*4 w(0:2*n)
      ...
      do i=0,n
         call fd_weights_full_f(z(i),z,n,1,w)
         do j=0,n
            d(i,j) = w(j+np)                   !  Derivative matrix
         enddo
      enddo
...
c-----------------------------------------------------------------------
      subroutine fd_weights_full_f(xx,x,n,m,c)
c
c     This routine evaluates the derivative based on all points
c     in the stencils.  It is more memory efficient than "fd_weights"
c
